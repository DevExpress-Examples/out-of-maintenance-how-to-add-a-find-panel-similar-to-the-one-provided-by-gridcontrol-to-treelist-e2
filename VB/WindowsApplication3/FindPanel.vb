...
Namespace DXSample
	...
	Public Class FindPanel
		...
		Private searchText_Renamed As String = String.Empty

		Public Sub New(ByVal provider As OptionsFindProvider, ByVal tree As TreeList)
		...
		Friend Property SearchText() As String
			...
			Set(ByVal value As String)
				If searchText_Renamed <> value Then
					searchText_Renamed = value
					If searchText_Renamed <> searchEdit.Text Then
						searchEdit.EditValue = searchText_Renamed
					End If
					If provider.AllowFilter Then
						FilterTree()
					ElseIf provider.HighlightFindResults Then
						InvalidateTree()
					End If
				End If
			End Set
		...
		Private Sub OnCustomDrawNodeCell(ByVal sender As Object, ByVal e As CustomDrawNodeCellEventArgs)
			If (Not provider.HighlightFindResults) Then
				Return
			End If
			If (Not SearchText.Equals(String.Empty)) AndAlso (provider.FindFilterColumns.Contains(e.Column.FieldName) OrElse provider.FindFilterColumns.Equals("*")) AndAlso ContainsSearchText(e.Column, e.Node) Then
		...
		Friend Sub FilterTree()
			tree.FilterNodes()
		End Sub

		Friend Sub InvalidateTree()
